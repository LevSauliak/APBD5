...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        // not effecient
        // List<Emp> result = emps.Where(emp => depts.Where(d => d.Loc == "CHICAGO").Select(d => d.DeptNo).Contains(emp.DeptNo)).ToList(); 
        var deptsInChicago = depts.Where(d => d.Loc == "CHICAGO").Select(d => d.DeptNo).ToHashSet();
        List<Emp> result = emps.Where(emp => deptsInChicago.Contains(emp.DeptNo)).ToList();
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (emp, dept) => new {DName = dept.DName, EName = emp.EName}).ToList(); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(e => e.Comm is not null).ToList(); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps
            .Where(
                e => e.Sal > emps.Where(emp => emp.DeptNo == e.DeptNo).Average(ee => ee.Sal))
            .Select(e => e.EName)
            .ToList();
